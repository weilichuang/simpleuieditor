...
package starling.textures
{
	...
	public class TextureAtlas
	{
		...
		private static var frame : Rectangle = new Rectangle();

		protected function parseAtlasXml( atlasXml : XML ) : void
		{
			var invScale : Number = 1 / _atlasTexture.scale;

			for each ( var subTexture : XML in atlasXml.SubTexture )
			{
				var name : String = StringUtil.clean( subTexture.@name );
				var x : Number = parseFloat( subTexture.@x ) * invScale;
				var y : Number = parseFloat( subTexture.@y ) * invScale;
				var width : Number = parseFloat( subTexture.@width ) * invScale;
				var height : Number = parseFloat( subTexture.@height ) * invScale;
				var frameX : Number = parseFloat( subTexture.@frameX ) * invScale;
				var frameY : Number = parseFloat( subTexture.@frameY ) * invScale;
				var frameWidth : Number = parseFloat( subTexture.@frameWidth ) * invScale;
				var frameHeight : Number = parseFloat( subTexture.@frameHeight ) * invScale;

...
package starling.textures
{
	...
	internal class ConcretePotTexture extends ConcreteTexture
	{
		...
		private var _potBase : flash.display3D.textures.Texture;
		...
		override protected function setBase( base : TextureBase ) : void
		{
			super.setBase( base );
			_potBase = this.base as flash.display3D.textures.Texture;
		}
		...
		{
			_potBase.uploadFromBitmapData( data );
			...
			if ( mipMapping && data.width > 1 && data.height > 1 )
			{
				...
				while ( currentWidth >= 1 || currentHeight >= 1 )
				{
					...
					_potBase.uploadFromBitmapData( canvas, level++ );
		...
		override public function uploadAtfData( data : ByteArray, offset : int = 0, async : Function = null ) : void
		{
			...
			_potBase.uploadCompressedTextureFromByteArray( data, offset, true );
			setDataUploaded();
		}

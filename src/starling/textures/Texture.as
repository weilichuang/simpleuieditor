...
package starling.textures
{
	...
	public class Texture
	{
		...
		public var formatBits : int = 0;
		public var format : String = Context3DTextureFormat.BGRA;

		/** Indicates if the texture contains mip maps. */
		public var mipMapping : Boolean = false;

		/** Indicates if the alpha values are premultiplied into the RGB values. */
		public var premultipliedAlpha : Boolean = false;

		/** The Stage3D texture object the texture is based on. */
		public var base : TextureBase = null;

		/** The concrete texture the texture is based on. */
		public var root : ConcreteTexture = null;
			...
			forcePotTexture : Boolean = false ) : Texture
		{
			...
			//useRectTexture = !forcePotTexture && !mipMapping &&
...
//				MatrixUtil.transformCoords( transformationMatrixToRoot, u, v, out );
				...
				//inline
				var transMat : Matrix = transformationMatrixToRoot;
				out.x = transMat.a * u + transMat.c * v + transMat.tx;
				out.y = transMat.d * v + transMat.b * u + transMat.ty;
			}
			return out;
		}
...
//				  MatrixUtil.transformCoords( sMatrix, u, v, out );
				...
				//优化代码
				var transMat : Matrix = transformationMatrixToRoot;
				var ta : Number = transMat.a;
				var tb : Number = transMat.b;
				var tc : Number = transMat.c;
				var td : Number = transMat.d;
				var ttx : Number = transMat.tx;
				var tty : Number = transMat.ty;

				var norm : Number = ta * td - tb * tc;
				if ( norm == 0 )
				{
					ta = tb = tc = td = 0;
					ttx = -ttx;
				...
				else
				{
					...
					var a1 : Number = td * norm;
					td = ta * norm;
					ta = a1;
					tb *= -norm;
					tc *= -norm;

					var tx1 : Number = -ta * ttx - tc * tty;
					tty = -tb * ttx - td * tty;
					ttx = tx1;
				}
		...
		public static function get maxSize() : int
		{
			...
			var profile : String = target ? target.painter.profile : "baseline";

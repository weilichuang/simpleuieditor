...
package starling.rendering
{
	...
	{
		starling_internal var _numVertices : int;
		...
		{
			sBytes.endian = Endian.LITTLE_ENDIAN;
		}
...
//			sBytes.clear();
			...
			vertexID : int = 0, numVertices : int = -1 ) : void
		{
			...
			tinted = true; // factor must be != 1, so there's definitely tinting.
		...
		public function createVertexBuffer( upload : Boolean = false, bufferUsage : String = "staticDraw" ) : VertexBuffer3D
		{
			...
			var buffer : VertexBuffer3D = Buffer3DManager.getInstance().createVertexBuffer( _numVertices, _vertexSize / 4, bufferUsage, BufferType.TYPE_2D );

...
package starling.utils
{
	...
	public class RenderUtil
	{
		...
		public static function getTextureFormatBits( format : String ) : int
		{
			var formatBits : int;
			switch ( format )
			{
		...
		private static var textureFlags : Array = [ "2d", "clamp", "dxt1", "nearest", "mipnone" ];

		public static function getTextureLookupFlags( format : String, mipMapping : Boolean,
			repeat : Boolean = false, smoothing : String = "none" ) : String
		{
			textureFlags[ 1 ] = repeat ? "repeat" : "clamp";
			...
			{
				textureFlags[ 3 ] = "nearest";
				textureFlags[ 4 ] = mipMapping ? "mipnearest" : "mipnone";
			}
			else if ( smoothing == TextureSmoothing.BILINEAR )
			{
				textureFlags[ 3 ] = "linear";
				textureFlags[ 4 ] = mipMapping ? "mipnearest" : "mipnone";
			}
			else
			{
				textureFlags[ 3 ] = "linear";
				textureFlags[ 4 ] = mipMapping ? "miplinear" : "mipnone";
			}

			return "<" + textureFlags.join() + ">";
		}
...
//
		...
		public static function createAGALTexOperation(
			...
			convertToPmaIfRequired : Boolean = true,
			mipMapping : Boolean = false, repeat : Boolean = false, smoothing : String = "bilinear",
			tempReg : String = "ft0" ) : String
		{
			var format : String = texture.format;
			var textureFlag : String = getTextureLookupFlags( format, mipMapping, repeat, smoothing );
			...
			var operation : String = "tex " + texReg + ", " + uvReg + ", fs" + sampler + textureFlag + "\n";

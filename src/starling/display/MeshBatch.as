...
package starling.display
{
	...
	public class MeshBatch extends Mesh implements IMeshBatch
	{
			...
			subset : MeshSubset = null, ignoreTransformations : Boolean = false ) : void
		{
			...
			var targetVertexID : int = vertexData.numVertices;
			var targetIndexID : int = indexData.numIndices;
			...
			if ( alpha != 1.0 )
				vertexData.scaleAlphas( "color", alpha, targetVertexID, subset.numVertices );
			if ( _parent )
				setRequiresRedraw();
		...
		{
			var numIndices : int = mesh.indexData._numIndices;
			var numVertices : int = mesh.vertexData._numVertices;
			var matrix : Matrix = mesh.transformationMatrix;
			var meshStyle : MeshStyle = mesh._style;

			if ( vertexData.numVertices == 0 )
				setupFor( mesh );
			...
			if ( alpha != 1.0 )
				vertexData.scaleAlphas( "color", alpha, vertexID, numVertices );
			if ( _parent )
				setRequiresRedraw();
		...
		{
			var currentNumVertices : int = vertexData.numVertices;

			if ( currentNumVertices == 0 )
				return true;
			if ( numVertices < 0 )
				numVertices = mesh.vertexData.numVertices;
			if ( numVertices == 0 )
				return true;
		...
		{
			if ( vertexData.numVertices != value )
			{
				vertexData.numVertices = value;
				_vertexSyncRequired = true;
				setRequiresRedraw();
			}
		}
		...
		{
			if ( indexData.numIndices != value )
			{
				indexData.numIndices = value;
				_indexSyncRequired = true;
				setRequiresRedraw();
			}
		}

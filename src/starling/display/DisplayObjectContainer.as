...
//            }
		...
		public function removeChildAt( index : int, dispose : Boolean = false ) : DisplayObject
		{
			if ( index >= 0 && index < _children.length )
			{
				...
				if ( index >= 0 )
				{
					StarlingVectorUtil.removeDisplayObjectAt( _children, index );
				}
				if ( dispose )
					child.dispose();
...
//				MatrixUtil.transformCoords(sHelperMatrix, localX, localY, sHelperPoint);
				...
				//copyFrom
				var transMat : Matrix = child.transformationMatrix;
				...
				//transformCoords
				sHelperPoint.x = ta * localX + tc * localY + ttx;
				sHelperPoint.y = td * localY + tb * localX + tty;
		...
		public override function render( painter : Painter ) : void
		{
			...
			for ( var i : int = 0; i < numChildren; ++i )
			{
				...
				if ( child._hasVisibleArea )
				{
					...
					if ( cacheEnabled && child._lastParentOrSelfChangeFrameID != frameID &&

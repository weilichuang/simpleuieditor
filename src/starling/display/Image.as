...
package starling.display
{
	...
	import starling.styles.GrayImageEffect;
	import starling.styles.GrayImageStyle;
	...
	public class Image extends Quad
	{
		...
		private var _curEffect : MeshEffect;
		private var _defaultEffect : MeshEffect;
		private var _grayEffect : GrayImageEffect;
		
		private var _defaultStyle : MeshStyle;
		private var _grayStyle : GrayImageStyle;
		private var _gray : Boolean = false;
		
		private var _batchable : Boolean = false;
		
		private var _vertexSyncRequired : Boolean = true;
		private var _indexSyncRequired : Boolean = true;
		...
		private function updateStyle() : void
		{
			...
			{
				if ( _grayStyle == null )
					_grayStyle = new GrayImageStyle();

				setStyle( _grayStyle, true );
			}
			else if ( !_batchable )
			{
				if ( _defaultStyle == null )
					_defaultStyle = new MeshStyle();

				setStyle( _defaultStyle, true );
			}
			else
			{
				if ( _curEffect )
		...
		public override function render( painter : Painter ) : void
		{
			...
			else
			{
				...
				_style.updateEffect( _curEffect, painter.state );
				_curEffect.render( painter, 0, indexData._numTriangles );
			}
		}
			...
			mergeWithPredecessor : Boolean = true ) : void
		{
			...
			if ( !_batchable || _gray )
			{
				...
				{
					if ( _grayEffect == null )
						_grayEffect = style.createEffect() as GrayImageEffect;

					if ( _grayEffect.indexBuffer == null )
					{
						setVertexAndIndexDataChanged();
					}

					_curEffect = _grayEffect;
				}
				else
				{
					if ( _defaultEffect == null )
						_defaultEffect = style.createEffect();

					if ( _defaultEffect.indexBuffer == null )
					{
						setVertexAndIndexDataChanged();
					}

					_curEffect = _defaultEffect;
				}
				_curEffect.onRestore = setVertexAndIndexDataChanged;
			}
		}
		...
		{
			_curEffect.uploadVertexData( vertexData );
			_vertexSyncRequired = false;
		}

		private function syncIndexBuffer() : void
		{
			_curEffect.uploadIndexData( indexData );
